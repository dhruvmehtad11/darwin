...
class ClusterUtils:
    ...
    def __init__(self, env: ENV_TYPE, email: str = None):
        ...
        """
        self.env = env
        self.compute_sdk = ComputeCluster(env, email)
    ...
    def get_ray_dashboard_url(self, cluster_id: str):
        ...
        internal_dashboards_url = (
            self.compute_url + "/" + cluster_id + "/dashboards?internal=True"
        )
        retries = 0
        while retries < 3:
            try:
    ...
    def create_cluster_from_job_definition(
        ...
        compute_resp = requests.request(
            "POST", compute_app_layer, data=json.dumps(job_cluster_definition)
        )
        if not 200 <= compute_resp.status_code < 300:
            raise ClusterCreationFailed(
                f"Error in creating job cluster. Error is {compute_resp.text}"
            )
        else:
            if compute_resp.json()["status"] == "ERROR":
                raise ClusterCreationFailed(
                    f"Error in creating job cluster. Error is {compute_resp.json()['data']}"
                )
        data = compute_resp.json()["data"]
