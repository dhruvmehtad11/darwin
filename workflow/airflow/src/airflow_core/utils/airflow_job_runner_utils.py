...
from commons.constant.constants import Entity
...
def update_cluster_details_in_workflow(
    ...
    if response.status_code != 200:
        raise Exception(
            f"Failed to update cluster details in ElasticSearch: {response.text}"
        )
...
def get_updated_entry_point_cmd(
    ...
    if entry_point_cmd.startswith("papermill"):
        ...
        if dynamic_artifact and source_type == WORKSPACE:
            ...
            # For example if file_path is /home/ray/fsx/workspace/sarthak.mittal@dream11.com/Playground/default/test/main.ipynb then we need to get test/main.ipynb
