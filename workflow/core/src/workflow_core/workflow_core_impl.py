...
import pytz
from commons.models.dao_response import DaoResponse
from commons.models.identifier import Identifier
...
from commons.models.search_query import SearchQuery
...
class WorkflowCoreImpl(WorkflowCoreInterface):
    ...
    def create_workflow(self, workflow_request: CreateWorkflowRequest, user_email: str):
        ...
        if find_resp is None or find_resp.data is None or len(find_resp.data) == 0:
            ...
            if create_resp.status == ERROR:
                self.logger.error(f"Creating workflow failed with response {create_resp}")
    ...
    async def create_workflow_v2(self, workflow_request: CreateWorkflowRequest, user_email: str):
        ...
        if find_resp is None or find_resp.data is None or len(find_resp.data) == 0:
            ...
            if create_resp.status.lower() == ERROR:
                self.logger.error(f"Creating workflow failed with response {create_resp}")
                raise Exception(create_resp)
            if create_resp.data.schedule == DEFAULT_SCHEDULE:
                create_resp.data.schedule = ""
    ...
    def activate_dag_v2(self, workflow_name: str, status: str = ACTIVE):
        ...
        if resp['is_paused']:
            return ResumeScheduleWorkflowIdPutResponseData(workflow_name=workflow_name, workflow_status=INACTIVE)
        else:
            return ResumeScheduleWorkflowIdPutResponseData(workflow_name=workflow_name, workflow_status=ACTIVE)
    ...
    async def soft_delete_workflow_by_id(self, workflow_id: str):
        ...
        if existing_workflow_resp.status == ERROR:
    ...
    async def run_now_workflow(self, workflow_id: str, user_email: str, parameters: dict = None):
        ...
        resp = self.airflow.run_now(wf.workflow_name, parameters)
    ...
    def parse_date(self, date_str):
        # Parse date in the format 2024-06-28T23:59:59Z
        try:
            # TODO add the parser for other date formats
    ...
    def update_cluster_details_in_workflow(self, workflow_cluster_request: WorkflowTaskClusterRequest):
        ...
        if existing_workflow_cluster is None:
            ...
            if update_resp.status == ERROR:
                self.logger.error(f"failed to update an entry for cluster, response: {update_resp}")
                raise Exception(update_resp)
            else:
                return WorkflowTaskClusterResponseData(status=update_resp.status,
