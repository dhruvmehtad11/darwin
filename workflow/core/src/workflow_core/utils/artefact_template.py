...
class Artefact(ABC):
    ...
    def create_operator(self, json):
        ...
        :return: None
        """
        task_flags = determine_kill_and_cluster_parent(json['tasks_definitions'],json['tasks_dependencies'])
        # Identify start tasks (those that are not a child in any dependency)
        all_task_ids = {task_def['task_id'] for task_def in json['tasks_definitions']}
        child_task_ids = {child for _, child in json['tasks_dependencies']}
        ...
        for task_def in json['tasks_definitions']:
            ...
            dag_id = json["dag_args"]["dag_id"]
            workflow_params = json['dag_args']['parameters']
