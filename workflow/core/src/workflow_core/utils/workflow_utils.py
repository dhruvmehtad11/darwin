...
from commons.kafka.event_entity import StateSubclass, Event
...
from datetime import datetime as dd
from commons.constant.constants import State, Entity
...
# Note: Entity.WORKFLOW is already defined in commons.constant.constants.Entity, so extend_enum is not needed
...
def _create_variable(key, value, env):
    ...
    response = requests.post(variables_url, headers=headers, json=variable_data, auth=auth)
    return response
...
def _get_variable(key, env):
    ...
    variables_url = f"{AIRFLOW_URL}/api/v1/variables"
    headers = {'Content-Type': 'application/json'}
    response = requests.get(f"{variables_url}/{key}", headers=headers, auth=auth)
    return response
...
def _delete_variable(key, env):
    ...
    variables_url = f"{AIRFLOW_URL}/api/v1/variables"
    headers = {'Content-Type': 'application/json'}
    response = requests.delete(f"{variables_url}/{key}", headers=headers, auth=auth)
    return response
...
def create_json_from_dag_args(env, dag_args, tasks, dependencies):
    ...
    data = f"json = {json.loads(json_data)}"
...
async def wait_for_api_success_v2(dag_id: str, env: str, message: str, interval: int = 1):
    status = True
    done = False
    time_waited = 0
    wait_timeout = 600
