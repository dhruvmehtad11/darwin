...
class AirflowApi:
    ...
    def get_dags(self):
        ...
        resp = requests.request(method='GET', url=url, headers={'Authorization': AUTH_TOKEN})
    ...
    def get_dag(self, dag_id: str):
        ...
        if resp.status_code == 200:
            ...
            raise Exception("Error in fetching dag with id: " + dag_id)
    ...
    def pause_a_dag(self, dag_id: str):
        url = self.airflow_url + AIRFLOW_ENDPOINT + "/dags/" + dag_id
        resp = requests.request(method='PATCH', url=url, headers={'Authorization': AUTH_TOKEN}, json=PAUSE_RUN)
        if resp.status_code == 200:
            return resp.json()
        else:
            raise Exception
    ...
    def resume_a_dag(self, dag_id: str):
        url = self.airflow_url + AIRFLOW_ENDPOINT + "/dags/" + dag_id
        resp = requests.request(method='PATCH', url=url, headers={'Authorization': AUTH_TOKEN}, json=RESUME_RUN)
        if resp.status_code == 200:
            return resp.json()
        else:
    ...
    def stop_a_run(self, dag_id: str, run_id: str):
        ...
        headers = {'Authorization': AUTH_TOKEN}
        resp = requests.request(method='PATCH', url=url, headers=headers, json=STOP_RUN)
    ...
    def get_task_output_log(self, dag_id: str, run_id: str, task_id: str, try_number: int = 1):
        ...
        resp = requests.request(method='GET', url=url, headers={'Authorization': AUTH_TOKEN}, json={})
        if resp.status_code == 200:
    ...
    def delete_dag(self, dag_id: str):
        url = self.airflow_url + AIRFLOW_ENDPOINT + "/dags/" + dag_id
        resp = requests.request(method='DELETE', url=url, headers={'Authorization': AUTH_TOKEN})
