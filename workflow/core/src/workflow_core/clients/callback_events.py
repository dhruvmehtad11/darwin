...
import requests
from commons.kafka.event_publisher import EventPublisher
...
class DarwinEventsListener(EventListener):
    ...
    def on_event(self, event: dict):
        headers = {
            ...
            'x-event-source': self.source
        }
        _request(method="POST", url=f"{self.base_url}/api/v1/event", data=json.dumps(event), headers=headers)
