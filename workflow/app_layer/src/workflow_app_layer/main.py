...
# e.g., if app is served at /workflow/, set ROOT_PATH=/workflow
root_path = os.environ.get("ROOT_PATH", "/workflow")
app = FastAPI(
    title='Workflow APIs',
    version='1.0.0',
    debug=True,
...
setup_v3_router(WorkflowCoreV3Impl())
setup_v3_runs_router(WorkflowRunV3Impl())
...
async def post_workflow(
    ...
    try:
        ...
        msd_user_dict = json.loads(msd_user)
        # Validate workflow asynchronously (uses asyncio.to_thread for blocking HTTP calls)
        await validate_workflow(body)
...
async def put_update_workflow(
    ...
    try:
        ...
        # Validate workflow asynchronously (uses asyncio.to_thread for blocking HTTP calls)
        await validate_workflow(body, find_resp.data[0])
